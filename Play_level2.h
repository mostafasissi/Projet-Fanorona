...
typedef struct  {
    position pionn;
    int directe;
    int nombre_pion_prise;
}prise;
typedef struct {
    position pionCourant;
    int directionCourant;
} prise_courant;
...
//===================================le  de l'ordinateur ========================================================
        ...
        else
        {
            contour=0;
            capture_poss=prise_possible(joueur_courant,  plateau );
            if (capture_poss==1){ // des prises possible
                 prise pion[30]; // declaration d'un tableau de prise
                 initialisetabprise(pion,30);
                 recherchePion(plateau,pion,1);
                pos->pos_i=pionETdirectionMax(pion,30).pionCourant;
                dir=pionETdirectionMax(pion,30).directionCourant;
            }
            else {// pas de prise possible => un déplacement simple
...
deplacement_simple:
        if (joueur_courant==j_humain  ){joueur_courant=j_machine;}
        else{joueur_courant=j_humain;}
